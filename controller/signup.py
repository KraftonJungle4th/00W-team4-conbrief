...
from clientConfigs.onboarding.CONFIGS import SIGN_UP
...
def signup():
    if request.method == 'GET':
        return render_template('onboarding/signUp.html', CONFIG=SIGN_UP)
    ...
    if request.method == 'POST':
        ...
        if not signupRequestDto['studentNo'] or len(signupRequestDto['password']) < 4 or len(signupRequestDto['password-check']) < 4:
            return render_template('onboarding/signUp.html', CONFIG=SIGN_UP, errorMessage="입력값을 확인해주세요")
        
        if signupRequestDto['password'] != signupRequestDto['password-check']:
            return render_template('onboarding/signUp.html', CONFIG=SIGN_UP, errorMessage="두 비밀번호가 일치하지 않습니다.")
